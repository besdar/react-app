...
export class Input extends Component {
    ...
    render() {
        return (<React.Fragment>
            {
                    this.props.child &&
                    <div style={{alignItems: 'center', backgroundColor: 'white', display: 'flex'}}>
                        <FaReply style={{fontSize: 'xx-large', margin: '10px'}} />
                       <div style={{flexDirection: 'column'}}>
                           <div>{this.props.child.title}</div>
                           <div>{this.props.child.text}</div>
                       </div>
                    </div>
                }
