...
const Paradocs:React.FC<PropsType> = (props) => {
    ...
    const footer = (
        <React.Fragment>
            <Button label="Да" icon="pi pi-check" onClick={SaveEditorsOnClick} />
    ...
    return (
        <div style={{ display: "flex", padding: "5px" }}>
            <Dialog header="Измененный текст не сохранен" footer={footer} visible={props.showPopup} style={{ width: "fit-content" }} modal={true} onHide={() => props.DialogOnExit()}>
                Сохранить измененный текст?
            </Dialog>
            <div style={{ flexBasis: "30%" }}>
                <Header SaveEditorsOnClick={SaveEditorsOnClick} />
                <Tree
                    value={props.items}
                    style={{ width: '95%' }}
